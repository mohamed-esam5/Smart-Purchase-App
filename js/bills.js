...
// عناصر التحكم بوضع التعديل المضافة حديثاً
const editBillIdInput = document.getElementById('edit-bill-id');
const modalTitleText = document.getElementById('modal-title-text');
const btnSubmitBill = document.getElementById('btn-submit-bill');
...
// 👥 جلب الأشخاص وتوليد خانات اختيار ومبالغ ديناميكية مع دعم ملء البيانات في وضع التعديل
async function loadPersonsWithAmountInputs(existingBreakdown = null) {
    ...
    try {
        ...
        querySnapshot.forEach((doc) => {
            ...
            // دمج البيانات القديمة إذا كنا في وضع التعديل الكامل للكارت
            if (existingBreakdown && existingBreakdown[person.name] !== undefined) {
                checkbox.checked = true;
                amountInput.disabled = false;
                amountInput.value = existingBreakdown[person.name];
            }
...
// 🔀 دالة الفرز والترتيب المحدثة والعرض الأنيق بأسلوب مصغر موحد
function renderAndSortBills() {
    ...
    cachedActiveBills.forEach(bill => {
        ...
        row.innerHTML = `
            ...
            <td>${statusBadge}</td>
            <td>
                <button class="btn-action-edit" data-id="${bill.id}"><i class="fa-solid fa-pen-to-square"></i> تعديل</button>
...
// 💾 حفظ الفاتورة (إضافة جديدة بالكامل أو تحديث شامل لبيانات وثيقة حالية)
billForm.addEventListener('submit', async (e) => {
    e.preventDefault();
    
    const editId = editBillIdInput.value;
    ...
    try {
        if (editId) {
            // 🔄 عملية التعديل في Firestore على الفاتورة الحالية بالكامل
            const billDocRef = doc(db, 'users', currentUserId, 'bills', editId);
            
            // جلب الفاتورة السابقة أولاً للحفاظ على الحسابات المسددة سابقاً إذا لزم الأمر أو تصفيرها بناءً على التعديل الجديد للمشتريات
            await updateDoc(billDocRef, {
                ...
                paymentMethod, 
                breakdown,     
                partiallyPaid, // تتم إعادة تهيئتها للسلامة الحسابية عند تعديل بنود الفاتورة بالكامل
                status: 'غير مدفوع'
            });
            alert("✅ تم تعديل بيانات الفاتورة وإعادة احتساب الأرصدة بنجاح.");
        } else {
            // 📥 عملية إضافة فاتورة جديدة تماماً
            const billsRef = collection(db, 'users', currentUserId, 'bills');
            ...
            alert("✅ تم حفظ الفاتورة الجديدة بنجاح.");
        }
...
// 🔄 إدارة أحداث التعديل والأرشفة داخل الجدول واقتناص الـ IDs
function addButtonListeners() {
    // حدث التعديل الكامل للبيانات
    document.querySelectorAll('.btn-action-edit').forEach(btn => {
        btn.addEventListener('click', async (e) => {
            const id = e.currentTarget.getAttribute('data-id');
            const bill = cachedActiveBills.find(b => b.id === id);
            
            if (!bill) return;

            // تغيير نصوص وعناوين المودال لتعبر عن التعديل الشامل
            editBillIdInput.value = bill.id;
            modalTitleText.innerHTML = `<i class="fa-solid fa-pen-to-square"></i> تعديل كامل بيانات الفاتورة`;
            btnSubmitBill.innerText = "تحديث الفاتورة وحفظ التعديلات";

            // ملء حقول المودال الأساسية ببيانات الفاتورة الحالية
            document.getElementById('bill-title').value = bill.title;
            document.getElementById('bill-amount').value = bill.amount;
            document.getElementById('purchase-date').value = bill.purchaseDate;
            document.getElementById('bill-payment-method').value = bill.paymentMethod || 'Cash';

            // تحميل قائمة الأشخاص وحقن مبالغهم المسجلة بداخلها تلقائياً
            await loadPersonsWithAmountInputs(bill.breakdown);

            // فتح المودال فورا للمستخدم
...
// 🕹️ تهيئة وتجميع كافة مستمعي الأحداث والفرز لايف
function initEvents() {
    if (sortBillsSelect) {
    ...
    if (openModalBtn) {
        openModalBtn.onclick = () => {
            // تصفير الحقل الخفي وإرجاع نصوص المودال لحالتها الافتراضية (إضافة جديدة)
            editBillIdInput.value = "";
            modalTitleText.innerHTML = `<i class="fa-solid fa-circle-plus"></i> إضافة فاتورة جديدة وتوزيع المبالغ`;
            btnSubmitBill.innerText = "حفظ الفاتورة واحتساب الاستحقاق";
            billForm.reset();
